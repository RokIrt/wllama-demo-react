...
// single files must be < 2 GB (ArrayBuffer limit; splits with < 2 GB shards also
// work) and total model size is bounded by the 4 GB WASM memory ceiling.
// Gemma 4 E2B is included via a custom split (888rok/gemma-4-E2B-it-wllama-split)
// since no publisher ships a wllama-compatible file. Not includable as of
// Aug 2026: Gemma 4 E4B and up (over the file/heap limits even at IQ2),
// LFM2.5-8B-A1B (smallest quant is a 2.57 GB single file), gpt-oss-20b and all
// 9B+ dense / large MoE models (exceed the 4 GB WASM heap).
